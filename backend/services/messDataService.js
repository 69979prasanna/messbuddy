...
/**
 * Format a 24-hour time string (HH:MM) to 12-hour format
 */
...
/**
 * Convert HH:MM into a Date object for today
 */
...
/**
 * Gather complete MessBuddy database facts to build anti-hallucination context
 */
export const getMessBuddyDataContext = async (userId = null) => {
  ...
  // Inspect each restaurant's schedule
  ...
  // Determine current active meal & next meal based on standard periods
  ...
  // 6. Format Verified Text Context for LLM
  ...
  const reviewsSummary = reviews.length > 0
    ? reviews.slice(0, 30).map((rev) => {
        // Find restaurant name if place matches restaurant id
